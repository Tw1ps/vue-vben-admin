...
import { deleteBotApi } from '@/api/st/bot';
import { Operator, Search } from '@/api/st/model/base';
import { Bot, BotColumns } from '@/api/st/model/bot';
...
export const requestDataForDelete: Search<BotColumns> = reactive({
  offset: 0,
  limit: 1,
  order: null,
  order_field: null,
...
const sendDeleteRequest = async (obj: Search<BotColumns>) => {
  try {
    await deleteBotApi(obj);
...
export const deleteOne = async (obj) => {
  await sendDeleteRequest({
    offset: 0,
    limit: 1,
    fields: [{ key: BotColumns.ID, op: Operator.EQ, value: obj.id }],
    order_field: null,
    order: null,
  });
...
export const deleteSelected = async () => {
  ...
  await sendDeleteRequest({
    offset: 0,
    limit: 1,
    fields: [{ key: BotColumns.ID, op: 'IN', value: tmpIDs }],
    order_field: null,
    order: null,
  });
