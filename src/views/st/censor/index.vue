<template>
  <PageWrapper contentFullHeight title="审查页" content="在此处推送需要审查的消息">
    <div class="!my-2 w-full">
      <Card size="small" :bordered="false">
        <Collapse ghost>
          ...
          <CollapsePanel key="2" header="工具栏">
            <Card size="small" class="text-right" :bordered="false"
              ><Space>
                <Dropdown :trigger="['click']">
                  <a class="ant-dropdown-link" @click.prevent>
                    <CommentOutlined />{{ t('st.base.pull') }}已选
                  </a>
                  <template #overlay>
                    <Menu>
                ...
                <a><DeleteOutlined />删除已选</a>
                <a><DeleteOutlined />删除全部</a>
                <a><DeleteOutlined />删除筛选</a>
              </Space></Card
            >
          </CollapsePanel>
        </Collapse>
      ...
      <Card title="使用说明" :bordered="false">使用说明</Card>
    </div>
    <Modal v-model:visible="visible" class="w-auto h-auto">
      <Card size="small" :bordered="false" class="w-auto h-auto" v-if="!visibleSource">
        <Descriptions class="p-6" :title="showModalField!.title" :column="1">
          <DescriptionsItem :label="t('st.columns.id')">{{ showModalField!.id }}</DescriptionsItem>
...
<script lang="ts" setup>
  ...
  let search: Search<CensorColumns> = reactive({
    offset: 0,
    limit: 20,
    order: null,
    order_field: null,
  ...
  const showModalField = ref<Censor>();
  const showModal = (obj: Censor) => {
  ...
  const doSearch = async () => {
    search.offset = 0;
    await getCensorData(search);
  };

  const doDeleteOne = async (v: Censor) => {
    await deleteCensor({
      offset: 0,
      limit: 1,
      fields: [{ key: CensorColumns.ID, operator: Operator.EQ, value: v.id }],
      order_field: null,
      order: null,
    });
  };
  ...
  const deleteCensor = async (obj: Search<CensorColumns>) => {
    try {
  ...
  const getCensorData = async (v: Search<CensorColumns>) => {
    ...
    setLoading(true);
    try {
      const ret = await getCensorApi(v);
