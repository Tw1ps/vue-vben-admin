...
// 编辑标签相关
export const visibleEditTagModal = ref<boolean>(false); // 展示编辑标签框开关

export const showModalForEditTag = () => {
  visibleEditTagModal.value = !visibleEditTagModal.value;
}; // 展示更新框开关函数

export const requestDataForEditTag: Search<HostColumns> = reactive({
  offset: 0,
  limit: 1,
  order: null,
  order_field: null,
...
let requestDataForNewTag: Array<Field<HostColumnsUpdate>> = reactive<
  Array<Field<HostColumnsUpdate>>
>([]);
export const newTags = ref<Array<string>>([]);

export const EditTag = (id: number, tags: Array<string>) => {
  // clearUpdateCondition();
...
export const sendUpdateRequestForTag = async () => {
  requestDataForNewTag = [{ key: HostColumnsUpdate.TAG, value: newTags.value }];
