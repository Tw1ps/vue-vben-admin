...
import { deleteKeywordApi } from '@/api/st/keyword';
import { Operator, Search } from '@/api/st/model/base';
import { Keyword, KeywordColumns } from '@/api/st/model/keyword';
...
export const requestDataForDelete: Search<KeywordColumns> = reactive({
  offset: 0,
  limit: 1,
  order: null,
  order_field: null,
...
const sendDeleteRequest = async (obj: Search<KeywordColumns>) => {
  try {
    await deleteKeywordApi(obj);
...
export const deleteOne = async (obj) => {
  await sendDeleteRequest({
    offset: 0,
    limit: 1,
    fields: [{ key: KeywordColumns.ID, op: Operator.EQ, value: obj.id }],
    order_field: null,
    order: null,
  });
...
export const deleteSelected = async () => {
  ...
  await sendDeleteRequest({
    offset: 0,
    limit: 1,
    fields: [{ key: KeywordColumns.ID, op: 'IN', value: tmpIDs }],
    order_field: null,
    order: null,
  });
