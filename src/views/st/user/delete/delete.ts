...
import { deleteUserApi } from '@/api/st/user';
import { Operator, Search } from '@/api/st/model/base';
import { User, UserColumns } from '@/api/st/model/user';
...
export const requestDataForDelete: Search<UserColumns> = reactive({
  offset: 0,
  limit: 1,
  order: null,
  order_field: null,
...
export const deleteOne = async (obj) => {
  await sendDeleteRequest({
    offset: 0,
    limit: 1,
    fields: [{ key: UserColumns.ID, op: Operator.EQ, value: obj.id }],
    order_field: null,
    order: null,
  });
...
export const deleteSelected = async () => {
  ...
  await sendDeleteRequest({
    offset: 0,
    limit: 1,
    fields: [{ key: UserColumns.ID, op: 'IN', value: tmpIDs }],
    order_field: null,
    order: null,
  });
